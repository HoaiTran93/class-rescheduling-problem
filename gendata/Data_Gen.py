...
def gen_data(dirPath, course_num, num_teacher, min_class, max_class, recruit_student, student_min_in_class, student_in_class, max_reg_a_class):
    ...
    for y in range(num_teacher):
        ...
        #max_pref = r.randrange(1, gen_teachers.at[y, 'Max Classes'] + 1)
...
student_min_in_class=30
student_in_class=30
max_reg_a_class=30

recruit_student = course_num * max_reg_a_class
...
for i in range(num_dataset) :
    dirPath = os.path.join(directory, 'dataset-hungarian-max' + str(i + seq).zfill(2))
    try: 
        os.stat(dirPath)
    except:
        os.mkdir(dirPath)
