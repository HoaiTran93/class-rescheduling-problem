...
######## Generator create data for steel annealing algorithm #########
...
def gen_data(dirPath, course_num, num_teacher, min_class, max_class, recruit_student, student_min_in_class, student_in_class, max_reg_a_class):
    ...
    for y in range(num_teacher):
        ...
        #r.randrange(1, gen_teachers.at[y, 'Max Classes'] + max_class)
        max_pref = course_num - r.randrange(1, course_num - max_class)
    ...
    for x in range(course_num):
        ...
        for y in range(assign_num_teacher):
            ...
            genr = 0 if (y <= x) else 1
            if genr == 0: #assign_course_num > list_len and r.randrange(1,4) % 2 == 0:
                if list_len > 0:
                ...
                t = gen_teachers.at[y, 'Max Classes'] if (r.randint(0, 2) % 2 == 0) else -gen_teachers.at[y, 'Max Classes']
                if (list_len > 0 and y > z and (x >= (y%h + t))):
                    index = r.randint(0, list_len)
                    assigned = int(assign_class.pop(index)) if list_len != index else int(assign_class.pop(0))
                    #assigned = int(assign_class.pop(0))
                    registration[teacher_key] = assigned
    ...
    print('################################################################')
    print('Total number of classes (teachers):' +  str(gen_teachers['Max Classes'].sum()))
    print('Total number of classes (courses):' +  str(gen_classes['No. Classes'].sum()))
    print('################################################################')
    
seq = int(input('Enter seq: '))
...
student_min_in_class=15
student_in_class=30
max_reg_a_class=80

recruit_student = course_num * r.randint(student_in_class, max_reg_a_class)
...
for i in range(num_dataset) :
    dirPath = os.path.join(directory, 'dataset-lk' + str(i + seq).zfill(2))
    try: 
        os.stat(dirPath)
    except:
        os.mkdir(dirPath)
        
    gen_data(dirPath, course_num, num_teacher, min_class, max_class, recruit_student, student_min_in_class, student_in_class, max_reg_a_class)
